...
"""
Created on Mon Nov 15 10:13:28 2021

Modification of Sklearn's PowerTransformer Class to allow for a temperature
dependent Yeo-Johnson transform; 
...
class PowerTransformerVariableLambda(PowerTransformer):
    ...
    **kwargs : 
    ...
    def fit_fmin(self, X, X_func, n_index):
        ...
        self.coeffs_ = np.array(Parallel(n_jobs=-1,verbose=0)(delayed(self._yeo_johnson_optimize_fmin)(X[:,i_grid], X_func[:,i_grid]) for i_grid in tqdm(np.arange(n_index),  desc = 'Optimizing Fmin:', position = 0, leave = True, file=sys.stdout)))
    ...
    def _yeo_johnson_optimize_fmin(self, x, x_func):
        ...
        return minimize(_neg_log_likelihood, np.array([0.001, 0.001]), bounds=bounds, method='SLSQP', jac=rosen_der
                        ).x
    ...
    def _yeo_johnson_transform_fmin(self, x, lmbda):
        ...
        out[pos_c] = -(np.power(-x[pos_c] + 1, 2 - lmbda[pos_c]) - 1) / (2 - lmbda[pos_c])
    ...
    def _yeo_johnson_inverse_transform_fmin(self, x, lmbda):
        """Apply the inverse power transformation using the fitted lambdas.
        The inverse of the Yeo-Johnson transformation is given by::
            if X >= 0 and lambda_ == 0:
                X = exp(X_trans) - 1
        ...
        # assign values for the four cases
        out[pos_a] = np.exp(x[pos_a]) - 1
        out[pos_b] = np.power(x[pos_b] * lmbda[pos_b] + 1,   1 / lmbda[pos_b]) - 1
        out[pos_c] = 1 - np.power(-(2-lmbda[pos_c]) * x[pos_c] + 1, 1 / (2 - lmbda[pos_c])) 
        out[pos_d] = 1 - np.exp(-x[pos_d])
    ...
    def inverse_transform_fmin(self, X, X_func):
        ...
        for i, lmbda in enumerate(lambdas.T):
            X_inv[:, i]  = self._yeo_johnson_inverse_transform_fmin(X[:, i], lmbda)
            X_inv[:, i]  = np.where(X_inv[:,i]<self.mins_[i], self.mins_[i], X_inv[:,i])
            X_inv[:, i]  = np.where(X_inv[:,i]>self.maxs_[i], self.maxs_[i], X_inv[:,i])
